...
def filter(df: pd.DataFrame, criteria: dict):
    ...
    filtered["1 Year Yield"] = df["1 Year Yield"]
    filtered["year delta"] = df["year_delta"]
...
def show_popup(win=None, msg="Popup Message"):
    ...
    if win == None:
        popup = tk.Tk()
        popup.title("Popup")
        ttk.Label(popup, text=msg, font=("Helvetica")).pack(padx=10, pady=10, expand=True)
        ttk.Button(popup, text="Done", font="Helvetica", command=popup.destroy).pack(padx=10, pady=10)
        popup.mainloop()
    ...
    # popup given window, continues the other tasks
        ...
        top = tk.Toplevel(win)
        top.title("Popup")
        ttk.Label(top, text=msg, font=("Helvetica")).pack(padx=10, pady=10, expand=True)
        ttk.Button(top, text="Done", font="Helvetica", command=top.destroy).pack(padx=10, pady=10)
...
class NEPSE:
    def __init__(self):
        ...
        for sectorRow in soup.find_all(attrs ={"class":"panel-default"}):
            sector = sectorRow.find('a').get_text().strip()
            try:
                companyRows = sectorRow.find_all("tr")[1:]
            except TypeError:
                print("error in finding sectors")
                continue
            self.companies[sector] = [row.find('a').get_text().strip() for row in companyRows]
            
        self.sectors = list(self.companies.keys())
...
class Company(NEPSE):
    def __init__(self, smbl):
        ...
        # setting the detailsf        
        detail_df = dfs[0].iloc[0:12] #only these are organized

        self.details = detail_df.set_index(0).to_dict()[1]
        self.details["1 Year Yield"] = self.details["1 Year Yield"].strip("%")
        self.details["Symbol"] = smbl
        self.details["EPS"] = float(self.details["EPS"].split()[0])
        [self.details["year_high"], self.details["year_low"]] = self.details["52 Weeks High - Low"].split("-")
        
        del self.details["Last Traded On"]
        del self.details["52 Weeks High - Low"]
        del self.details["% Change"]
        ...
        self.details["year_delta"] = self.details["year_high"] - self.details["year_low"]
...
class MyGUI:
    def __init__(self):
        ...
        # window
        self.window = tk.Tk()  
        self.window.state('zoomed')  # Windows only
    ...
    def render_download_frame(self):
        self.download_frame = ttk.Frame(self.window)
        ...
        self.download_sector = ttk.StringVar(self.download_frame)
        self.download_sector.set("All")
        self.drop_sector = ttk.OptionMenu(
    ...
    def render_filter(self):
        ...
        self.filter_band = ttk.Frame(self.window)
        self.filter_frame = ttk.Frame(
            self.filter_band
        )

        # filter:sector
        self.query_sector = ttk.StringVar(self.filter_frame)
        self.query_sector.set("No data found")  # set as No data found till updated
        ttk.Label(self.filter_frame, text="Sector").grid(
            row=0, column=0, padx=2, sticky="NEWS"
        )
        self.selector_sector = ttk.OptionMenu(
            self.filter_frame, self.query_sector, "No data found"
        )
        self.selector_sector.grid(row=1, column=0, padx=2, sticky="NEWS")

        # filter:price
        ttk.Label(self.filter_frame, text="Price\n<").grid(
            row=0, column=1, padx=2, sticky="NEWS"
        )
        self.selector_price = ttk.Entry(self.filter_frame, width=8)
        self.selector_price.grid(row=1, column=1, padx=2, sticky="NEWS")

        # filter:book_val
        ttk.Label(self.filter_frame, text="Book_value\n<").grid(
            row=0, column=2, padx=2, sticky="NEWS"
        )
        self.selector_book_val = ttk.Entry(self.filter_frame, width=8)
        self.selector_book_val.grid(row=1, column=2, padx=2, sticky="NEWS")

        # filter:pbv
        ttk.Label(self.filter_frame, text="PBV\n<").grid(
            row=0, column=3, padx=2, sticky="NEWS"
        )
        self.selector_PBV = ttk.Entry(self.filter_frame, width=8)
        self.selector_PBV.grid(row=1, column=3, padx=2, sticky="NEWS")

        # filter:eps
        ttk.Label(self.filter_frame, text="EPS\n>").grid(
            row=0, column=4, padx=2, sticky="NEWS"
        )
        self.selector_eps = ttk.Entry(self.filter_frame, width=8)
        self.selector_eps.grid(row=1, column=4, padx=2, sticky="NEWS")

        # filter:PE ratio
        ttk.Label(self.filter_frame, text="PE ratio\n<").grid(
            row=0, column=5, padx=2, sticky="NEWS"
        )
        self.selector_PE = ttk.Entry(self.filter_frame, width=8)
        self.selector_PE.grid(row=1, column=5, padx=2, sticky="NEWS")

        # filter:dvnd_rate
        ttk.Label(self.filter_frame, text="dvnd %\n>").grid(
            row=0, column=6, padx=2, sticky="NEWS"
        )
        self.selector_dvnd_rate = ttk.Entry(self.filter_frame, width=8)
        self.selector_dvnd_rate.grid(row=1, column=6, padx=2, sticky="NEWS")

        # filter:dvnd_probability
        ttk.Label(self.filter_frame, text="dvnd prob.\n% >").grid(
            row=0, column=7, padx=2, sticky="NEWS"
        )
        self.selector_dvnd_prob = ttk.Entry(self.filter_frame, width=8)
        self.selector_dvnd_prob.grid(row=1, column=7, padx=2, sticky="NEWS")

        # filter:bonus_rate
        ttk.Label(self.filter_frame, text="Bonus %\n >").grid(
            row=0, column=8, padx=2, sticky="NEWS"
        )
        self.selector_bonus_rate = ttk.Entry(self.filter_frame, width=8)
        self.selector_bonus_rate.grid(row=1, column=8, padx=2, sticky="NEWS")

        # filter:bonus_probability
        ttk.Label(self.filter_frame, text="Bonus prob.\n% >").grid(
            row=0, column=9, padx=2, sticky="NEWS"
        )
        self.selector_bonus_prob = ttk.Entry(self.filter_frame, width=8)
        self.selector_bonus_prob.grid(row=1, column=9, padx=2, sticky="NEWS")

        # filter:year_change
        ttk.Label(self.filter_frame, text="Year Change %\n <, >, =S=").grid(
            row=0, column=10, padx=2, sticky="NEWS"
        )
        self.selector_year_change = ttk.Entry(self.filter_frame, width=8)
        self.selector_year_change.grid(row=1, column=10, padx=2, sticky="NEWS")
        
        # filter:year_delta
        ttk.Label(self.filter_frame, text="Year delta %\n <, >, ==").grid(
            row=0, column=11, padx=2, sticky="NEWS"
        )
        self.selector_year_delta = ttk.Entry(self.filter_frame, width=8)
        self.selector_year_delta.grid(row=1, column=11, padx=2, sticky="NEWS")
        ...
        self.button_filter.grid(
            row=0, column=12, padx=15, pady=10, sticky="NEWS", rowspan=2
        )

        self.filter_frame.pack()
        self.filter_band.pack(fill="x",pady=30)
    ...
    def render_order_frame(self):
        ...
        self.order_frame = ttk.Frame(self.window)
        self.order_label = ttk.Label(self.order_frame, text=f"Sort Order: ")
        self.order_label.pack(pady=5, side="left")

        # setting options
        self.order = ttk.StringVar(self.order_frame)
        self.order.set("Ascending")
        ttk.OptionMenu(self.order_frame, self.order, "Ascending", "Descending").pack(
            pady=5, side="right"
        )

        self.order_frame.pack(pady=30)
    ...
    def render_df(self, df: pd.DataFrame):
        ...
        self.df_frame.rowconfigure(0, weight=1)
        self.df_frame.columnconfigure(0, weight=1)
        
        #tree
        tree = ttk.Treeview(self.df_frame, columns=cols, show="headings", height=50)
        tree.grid(row=0, column=0,sticky="news")

        # scrollbar-vsb
        vsb = ttk.Scrollbar(self.df_frame, orient="vertical", command=tree.yview)
        vsb.grid(row=0, column=1, sticky="ns",padx=10,)
        ...
        self.df_frame.pack(expand=False,fill="both", pady=5,padx=50)
    ...
    def apply_fitler(self):
        while True:
            ...
            num_criteria = {
                    ...
                    "1 Year Yield": self.selector_year_change.get(),
                    "year_delta": self.selector_year_delta.get(),
                }
            ...
            num_criteria = {
                ...
                if value.strip() not in ("", "<", ">", "=") #no inputs are ignored
            }
